...
\begin{abstract}
Knowledge Tracing (KT) is the fundamental task of sequentially modeling student mastery over educational Knowledge Components (KCs) based on interaction logs, serving as the core engine for personalized learning pathways and learner-state estimation in Intelligent Tutoring Systems (ITS) and Educational Data Mining (EDM). Standard deep sequence models treat exercises as isolated categorical tokens, ignoring explicit domain structures and suffering representation collapse under interaction data sparsity. To address these limitations, Graph-Based KT (Graph-KT) and Self-Supervised KT (SSL-KT) have rapidly expanded. Following the PRISMA 2020 statement, this paper presents a systematic review of 37 primary core studies (2018--2026) retrieved across seven electronic databases ($N = 2,017$). We establish a two-dimensional taxonomy classifying Graph-KT along graph provenance (expert, log-based, dynamic, LLM-extracted) and GNN encoders (GCN, GAT, heterogeneous, hypergraph, dynamic), and SSL-KT across four pretext task families (multi-view, structural, hypergraph contrastive, generative masked auto-encoding). Methodologically, we expose a critical evaluation gap: 94.6\% of studies evaluate exclusively on generic interaction data sparsity where all KCs appear during training, whereas only 5.4\% implement strict zero-exposure concept cold-start splits. Standard models collapse near random guessing under zero-exposure splits because GNN message passing over co-occurrence graphs lacks inductive out-of-vocabulary mechanisms. Furthermore, 0 out of 37 studies report Expected Calibration Error (ECE) or evaluate probability calibration. We formulate six evidence-derived research agenda pillars with formal mathematical specifications---including LLM-GNN fusion, hyperbolic state manifolds, standardized cold-start benchmarks, and calibrated trustworthy AI---providing a methodological foundation for advancing adaptive learning technologies.
\end{abstract}

\begin{IEEEkeywords}
Knowledge Tracing, Graph Neural Networks, Self-Supervised Learning, Sparse Concepts, Educational Data Mining.
\end{IEEEkeywords}}

\maketitle
\begin{center}\textbf{BUILD-CANARY-E2}\end{center}
...
\caption{PRISMA 2020 Flow Diagram of Literature Search, Deduplication, Screening, and Eligibility Pipeline ($N = 37$ Primary Core Papers; raw retrieved $N_{\text{raw}} = 2,017$ across 7 electronic databases; mutually exclusive screening stages).}
...
Methodological quality was evaluated using an 8-item Quality Assessment rubric ($\text{QA1}--\text{QA8}$), scoring each dimension from $0.0$ to $2.0$. Non-sparse studies received $\text{NA}$ for $\text{QA7}$, bounding their max applicable score to $14.0$. Overall quality scores were categorized into two active tiers: \textbf{HIGH} ($\ge 80\%$) and \textbf{MODERATE} ($55\%--79\%$).
...
\caption{Two-Dimensional Taxonomy Framework for Graph-Based and Self-Supervised Knowledge Tracing Models ($N = 37$ Core Papers; Graph Provenance $N_G = 31$, SSL Pretext Families $N_S = 14$, Intersection $N_{G \cap S} = 8$).}
...
\caption{Four-Channel Graph Construction Pipeline in Graph-KT ($N_G = 31$ Graph-KT Papers; categories are mutually exclusive per study based on primary graph construction mechanism).}
...
\caption{Quantitative Meta-Analysis Distribution of Graph Neural Network (GNN) Encoder Architectures across Graph-KT Studies ($N_G = 31$; primary encoder categories are mutually exclusive: GCN $35.5\%$, GAT $22.6\%$, Heterogeneous GNN $19.4\%$, Hypergraph GNN $12.9\%$, Dynamic GNN $9.7\%$).}
...
\begin{table*}[t]
\caption{Taxonomy Table 1: Graph-Based Knowledge Tracing Models (RQ1 \& RQ2). Complete classification of 31 Graph-KT studies ($N_G = 31$) across Graph Provenance, Topology, GNN Encoder Architecture, Temporal Backbone, and State Fusion Mechanism (mutually exclusive primary categorizations).}
...
\caption{Taxonomy Framework of Self-Supervised Learning (SSL-KT) Pretext Objectives ($N_S = 14$ SSL-KT Papers; four pretext task families: Multi-View Contrastive $42.9\%$, Graph Structural Contrastive $28.6\%$, Hypergraph Contrastive $14.3\%$, Generative Masked Auto-Encoding $14.3\%$; categories are mutually exclusive per primary SSL objective).}
...
Eight primary core papers ($N_{G \cap S} = 8$) integrate both Graph Neural Networks and Self-Supervised contrastive objectives (e.g., Bi-CLKT \cite{KT011_BiCLKT_2022}, S2-HHN \cite{KT042_S2HHN_2023}, HyperKT \cite{KT066_HyperKT_2025}). Combining structural GNN propagation with self-supervised contrastive regularization achieves competitive predictive capacity. Table~\ref{tab:ssl_taxonomy} presents the taxonomy of all 14 SSL-KT studies.

\begin{table*}[t]
\caption{Taxonomy Table 2: Self-Supervised Knowledge Tracing Models (RQ3). Complete classification of 14 SSL-KT studies ($N_S = 14$) across SSL Family, Pretext Task Objective, Augmentation Strategy, Structure Preservation, Training Mode, and Auxiliary Loss Function.}
...
\begin{table*}[t]
\caption{Taxonomy Table 3: Sparse-Concept \& Cold-Start Evaluation Protocols (RQ4). Representative subset of 13 studies with explicit sparse-concept claims shown; remaining 24 core studies employ generic learner splits without sparse-concept claims ($N = 37$ Primary Core Papers; Interaction Sparsity $94.6\%$, Item Cold-Start $8.1\%$, Strict Zero-Exposure Concept Cold-Start $5.4\%$).}
...
\textbf{Contribution 2 --- Evaluation split audit.} We identified a pervasive methodological vulnerability: 94.6\% of studies claim to address data sparsity, yet only 5.4\% evaluate under a strict zero-exposure KC cold-start protocol. Standard graph architectures experience severe performance degradation under zero-exposure splits because GNN propagation over precomputed co-occurrence graphs lacks inductive out-of-vocabulary mechanisms. Furthermore, probability calibration remains completely unevaluated across published studies (0 out of 37 report ECE).

\textbf{Contribution 3 --- Evidence-derived research agenda.} Six concrete future research directions are proposed with formal mathematical specifications: LLM-GNN multimodal fusion, non-Euclidean hyperbolic state manifolds, standardized zero-exposure benchmarks (\textit{pyKT-ColdStart}), calibrated trustworthy KT, continuous-time spatiotemporal Neural ODEs, and causal counterfactual graph debiasing.
