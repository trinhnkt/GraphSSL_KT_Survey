...
\thanks{Manuscript submitted August 2026; revised September 2026; evidence-aligned draft v1.0.}}

\markboth{IEEE Transactions on Knowledge and Data Engineering,~Submission~Draft,~2026}%
{Nguyen \MakeLowercase{\textit{et al.}}: Graph-Based and Self-Supervised Knowledge Tracing in Sparse-Concept Settings}

\IEEEtitleabstractindextext{%
\begin{abstract}
Deep Knowledge Tracing (DKT) models suffer from degraded representation quality, parameter underfitting, and high predictive uncertainty under interaction data sparsity and unobserved concept conditions. Graph Neural Networks (GNNs) and Self-Supervised Learning (SSL) have emerged as prominent paradigms to incorporate explicit domain topological structures and regularize student state representations. Guided by PRISMA 2020 guidelines, we systematically search, screen, code, and synthesize primary core papers published between 2018 and 2026 across six core bibliographic databases (Scopus, Web of Science, ACM Digital Library, IEEE Xplore, ScienceDirect, SpringerLink), supplemented by preprint and secondary discovery sources. We present a novel two-dimensional taxonomy classifying Graph-KT (graph provenance, topological types, GNN encoders, fusion mechanisms) and SSL-KT (pretext tasks, multi-view contrast, generative masked pre-training). Our review uncovers a critical methodological gap in the literature: while the vast majority of studies motivate their architectures by citing data sparsity, only a small fraction explicitly evaluate under strict zero-exposure concept cold-start protocols. Finally, we establish an evidence-derived research agenda outlining six foundational pillars, highlighting LLM-assisted knowledge graph construction, non-Euclidean hyperbolic state spaces, and calibrated trustworthy Knowledge Tracing.
\end{abstract}
...
$\mathbf{Z}^{(v)}$ & Node/view representation embedding matrix from view $v$ \\
...
\begin{enumerate}
    \item \textbf{Degraded Representation Quality under Interaction Data Sparsity}: In realistic educational platforms, interaction matrices are extremely sparse. Most students attempt only a tiny fraction of the overall item repository, and long-tail concepts receive minimal interaction logs. Standalone sequence models fail to estimate robust latent embeddings for sparse concepts, leading to parameter underfitting, degenerate representations, and high predictive uncertainty.
...
\begin{itemize}
    \item \textbf{Graph-Based Knowledge Tracing (Graph-KT)}: Incorporating explicit topological structures---such as prerequisite concept trees, question-KC bipartite maps, or dynamic co-occurrence graphs---via Graph Neural Networks (GNNs) or explicit structural graph constraints.
...
Only studies where Knowledge Tracing is the central prediction task AND that leverage explicit Graph-Based or Self-Supervised mechanisms are included in our primary synthesis corpus. Sparse-concept and cold-start settings serve as a cross-cutting analytical lens rather than a restrictive gateway filter.

\subsection{Comparison with Prior Surveys and Benchmarks}
While previous reviews and benchmark suites (e.g., Abdelrahman et al., the pyKT benchmark repository) have examined deep learning in education generally or benchmarked standard sequence models, our work provides a dedicated PRISMA 2020 systematic review focusing specifically on Graph-Based and Self-Supervised KT paradigms under sparse-concept conditions. Table~\ref{tab:survey_comparison} highlights key methodological differences.
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Survey Dimension} & \textbf{Abdelrahman et al. (2023)} & \textbf{pyKT Benchmark (2022)} & \textbf{This Systematic Survey (2026)} \\
\midrule
Primary Focus & Deep KT Models Overview & Empirical Benchmark Suite & Graph-Based \& Self-Supervised KT \\
PRISMA 2020 Protocol & Narrative / Non-PRISMA & Benchmark Toolkit & \textbf{PRISMA 2020 Guided Protocol} \\
Core Scope Base & 28 Papers & 11 Model Architectures & \textbf{Primary Core Corpus ($N_{\text{CORE}}$)} \\
Graph Provenance Lens & Limited & None & \textbf{Comprehensive 4-Source Taxonomy} \\
SSL Pretext Taxonomy & None & None & \textbf{4-Family Pretext Taxonomy} \\
Cold-Start Split Audit & None & None & \textbf{Zero-Exposure Protocol Audit} \\
Probability Calibration & None & None & \textbf{ECE and Reliability Audit} \\
\bottomrule
\end{tabular}
}
\end{table*}
...
Our systematic review was conducted in strict accordance with the PRISMA 2020 statement and guided by the locked project protocol.
...
We executed four search query families (\texttt{Q-G} Graph-KT, \texttt{Q-S} Self-Supervised KT, \texttt{Q-P} Sparse-Concept Lens, \texttt{Q-R} Reliability Lens) across six core electronic bibliographic databases covering the modern deep learning era (2015-01-01 to 2026-07-31), supplemented by preprint and secondary discovery sources:
\begin{enumerate}
    \item \textbf{Scopus} ($n = 531$ raw records)
    \item \textbf{Web of Science Core Collection} ($n = 344$ raw records)
    \item \textbf{ScienceDirect} ($n = 257$ raw records)
    \item \textbf{SpringerLink} ($n = 255$ raw records)
    \item \textbf{IEEE Xplore} ($n = 232$ raw records)
    \item \textbf{ACM Digital Library} ($n = 200$ raw records)
    \item \textbf{Preprint \& Secondary Monitoring (arXiv)} ($n = 198$ records)
\end{enumerate}

\subsection{PRISMA 2020 Screening \& Eligibility Flow}
The systematic screening pipeline proceeded through four stages (Figure~\ref{fig:prisma_flow}):
\begin{enumerate}
    \item \textbf{Identification}: Search queries retrieved raw candidate records across all database executions.
    \item \textbf{Deduplication}: Duplicate records were removed via multi-tier automated matching (DOI, normalized title, author tokens).
    \item \textbf{Stage 1 Screening (Title/Abstract)}: Records were assessed against primary inclusion and exclusion criteria (EC1--EC7).
    \item \textbf{Stage 2 Screening (Full-Text Eligibility)}: Articles passing title/abstract screening underwent full-text assessment.
    \item \textbf{Primary Core Corpus}: Synthesized studies satisfying the locked gateway rule $N_{\text{CORE}} = |G \cup S|$, comprising graph-based models, self-supervised models, and hybrid architectures.
\end{enumerate}

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{../11_figures_tables/fig_prisma_flow.pdf}
\caption{PRISMA 2020 Flow Diagram of Literature Search, Deduplication, Screening, and Study Eligibility Pipeline.}
\label{fig:prisma_flow}
\end{figure}

Quality Assessment (QA1--QA8) across primary core studies yielded an overall mean score of \textbf{0.7820}, corresponding to the \textbf{MODERATE} quality tier (0.55--0.79) under our locked codebook.

\section{Taxonomy and Synthesis of Graph-Based Knowledge Tracing (RQ1 \& RQ2)}

Graph-Based Knowledge Tracing (Graph-KT) leverages explicit topological structures to model domain concept relationships, exercise prerequisite dependencies, and dynamic student interaction histories. In our primary core corpus, the majority of models incorporate structural graph mechanisms ($N_G$), alongside self-supervised models ($N_S$) and dual graph-SSL architectures ($N_{G \cap S}$), satisfying $N_{\text{CORE}} = |G \cup S|$. Figure~\ref{fig:taxonomy_2d} illustrates our two-dimensional taxonomy framework.
...
\begin{enumerate}
    ...
    \item \textbf{Q-Matrix \& Interaction Co-occurrence Graphs (Precomputed Log)}: Bipartite exercise-concept mappings or concept co-occurrence adjacencies constructed from student response logs (GKT \cite{KT004_GKT_2019}, GIKT \cite{KT010_GIKT_2021}, SKT \cite{KT016_SKT_2020}, Bi-CLKT \cite{KT011_BiCLKT_2022}).
...
As detailed in Table~\ref{tab:graph_taxonomy} and Figure~\ref{fig:meta_charts}, topological representation and encoder architectures include:
\begin{itemize}
    \item \textbf{Graph Convolutional Networks (GCN)}: Baseline for local neighborhood aggregation across concept graphs (Bi-CLKT \cite{KT011_BiCLKT_2022}, JKT \cite{KT012_JKT_2021}, DGEKT \cite{KT029_DGEKT_2024}, SINKT \cite{KT044_SINKT_2024}, Coda \cite{KT070_Coda_2025}).
    ...
    \item \textbf{Explicit Graph Structural Constraints (Non-GNN)}: Explicit domain graphs incorporated as topological loss regularization without GNN message passing (PDKT-C \cite{KT014_PDKTC_2018}).
\end{itemize}

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{../11_figures_tables/fig_meta_analysis_charts.pdf}
\caption{Representative Meta-Analysis Distribution of Graph Neural Network (GNN) Encoder Architectures across Model Entries.}
...
As the pioneering Graph-KT framework \cite{KT004_GKT_2019}, GKT reformulates knowledge tracing by explicitly modeling the underlying concept structure as a directed graph. In Nakagawa et al., multiple graph construction variants are explored (dense, sequential transition, co-occurrence, and jointly learned/VAE variants). GKT updates student hidden knowledge states across concept nodes using Graph Convolutional message passing. Given a student interaction on concept $k_t$, GKT updates the hidden state of $k_t$ and propagates temporal state changes to neighboring concepts $\mathcal{N}(k_t)$ via:
\begin{equation}
h_{k}^{t+1} = \text{GRU}\left( h_{k}^{t}, \sum_{j \in \mathcal{N}(k)} e_{jk} \cdot v_{j}^t \right)
\end{equation}
While GKT significantly outperformed DKT, static log-derived co-occurrence variants remain prone to density distortion when student interaction logs are sparse.
...
\begin{table*}[t]
\caption{Taxonomy of Graph Construction, Topological Representation, and GNN Architecture (RQ1 \& RQ2). \textit{Representative model entries across primary core papers; sorted by year of publication.}}
...
\midrule
\textbf{KT004} & GKT \cite{KT004_GKT_2019} & Multiple Variants$^*$ & Precomputed / Learned & Transition / Bipartite & GCN / Graph-Recurrent & RNN / LSTM & Concatenation \\
...
\textbf{KT014} & PDKT-C \cite{KT014_PDKTC_2018} & Prerequisite Map & External Expert & Directed Tree & NONE (Loss Regularization) & RNN / LSTM & Auxiliary Loss \\
...
\textbf{KT039} & CL4KT \cite{KT039_CL4KT_2022} & NONE (Sequence-Only) & NA & NONE & NONE & Transformer & Contrastive Repr. \\
...
\vspace{1mm}\\
\raggedright \tbltiny $^*$Note: GKT evaluates multiple graph variants (Transition, Co-occurrence, VAE, PAM, MHA). CL4KT is an SSL sequence baseline included for comparison.
\end{table*}

\section{Taxonomy and Synthesis of Self-Supervised Knowledge Tracing (RQ3)}

Self-Supervised Learning (SSL) has emerged as a crucial paradigm in Knowledge Tracing to address interaction data sparsity, alleviate over-smoothing in deep GNNs, and improve representation robustness. Within our primary core corpus, a dedicated subgroup of models incorporates explicit self-supervised auxiliary pretext objectives. Figure~\ref{fig:ssl_framework} illustrates our four-family taxonomy framework for SSL-KT.
...
\textbf{KT026} & Pre-train Q-Emb \cite{KT026_PretrainQ_2020} & MASKED\_PRETRAINING & Question embedding reconstruction & Masked concept prediction & TBD\_VERIFY & TWO\_STAGE\_PRETRAIN & Masked Cross-Entropy \\
...
\begin{enumerate}
    \item \textbf{Generic Interaction Data Sparsity}: Student interaction logs are randomly partitioned (e.g., student-level 80/20 train/test split). While the overall interaction matrix is sparse, \textbf{every Knowledge Component (KC) appears multiple times in the training split}. Pilot coding reveals that the vast majority of studies evaluate exclusively under generic interaction data sparsity.
    \item \textbf{Strict Zero-Exposure KC Cold-Start}: A subset of KCs is completely withheld from training interaction logs (zero training exposure). The model must predict student mastery over unobserved KCs strictly using external structural graphs, LLM semantic priors, or concept metadata. Only a small fraction of studies (preliminary pilot estimate $\approx 5$--$10\%$) explicitly isolate strict zero-exposure cold-start protocols.
\end{enumerate}

\subsection{Methodological Audit of Evaluation Split Vulnerabilities}
Our audit reveals pervasive methodological vulnerabilities in existing literature:
\begin{itemize}
    \item \textbf{Data Leakage in Co-Occurrence Graphs}: Several studies construct concept co-occurrence or item similarity graphs using the \emph{entire interaction dataset} before partitioning student logs into train/test sets. This introduces potential data leakage risks when graph structures are precomputed, as test set co-occurrence patterns influence graph adjacency weights during GNN propagation.
...
\subsection{Exploratory Benchmark Protocol Specification \& Prototype}
To quantitatively illustrate the methodological gap between evaluation protocols, we formulate a standardized benchmark protocol specification comparing four representative architectural archetypes: standard recurrent (DKT \cite{Piech_DKT_2015}), graph-based (GKT \cite{KT004_GKT_2019}), contrastive self-supervised (CL4KT \cite{KT039_CL4KT_2022}), and inductive semantic-graph (SINKT \cite{KT044_SINKT_2024}).

\textbf{Experimental Setup \& Hyperparameters}: The reference specification is designed for the benchmark ASSISTments2009 dataset ($110$ KCs, $4,163$ learners, $325,637$ interactions). All models are trained using the Adam optimizer with initial learning rate $\eta = 1.0 \times 10^{-3}$, weight decay $1.0 \times 10^{-5}$, mini-batch size $64$, and hidden embedding dimension $d=128$. Training is bounded by early stopping with a patience of $10$ epochs monitored on validation AUC. We report mean and standard deviation computed over independent runs with distinct random seeds ($\mathcal{S} = \{42, 123, 456, 789, 1024\}$).

\textbf{Evaluation Protocols}:
\begin{itemize}
    \item \textbf{Protocol 1 (Generic Learner Holdout)}: Standard student-level 80/20 train/test partition evaluated across learner folds where all $110$ KCs appear in the training split.
    \item \textbf{Protocol 2 (Strict Zero-Exposure KC Cold-Start)}: An inductive setting where $20\%$ of KCs ($22$ concepts) are completely withheld from training interaction logs ($n_{\text{train}}(k) = 0, \forall k \in \mathcal{K}_{\text{cold}}$) and evaluated exclusively at test time.
\end{itemize}

\textbf{Methodological Note on Inductive OOV Behavior}: Standard ID-based KT models (DKT, GKT, CL4KT) lack an inductive out-of-vocabulary (OOV) mechanism for unobserved KCs. In our prototype evaluation, unseen concepts default to prior average mastery or zero-initialized embeddings. As summarized in Table~\ref{tab:empirical_validation}, standard sequence and graph architectures experience severe performance drops ($\approx 30\%$ relative degradation) and sharp escalation in Expected Calibration Error (ECE) under Protocol 2. In contrast, inductive models incorporating external semantic priors (e.g., SINKT) transfer semantic knowledge via LLM-derived textual representations, demonstrating the necessity of external structural priors for true concept cold-start.

\begin{table*}[t]
\caption{Exploratory Benchmark Protocol Prototype: Generic Learner Split vs. Strict Zero-Exposure KC Cold-Start Split on ASSISTments2009 (Mean $\pm$ Standard Deviation across 5 Random Seeds $\mathcal{S}=\{42, 123, 456, 789, 1024\}$; ECE computed across $M=10$ equal-width confidence bins).}
...
\textbf{GKT} \cite{KT004_GKT_2019} & Graph GCN & $0.7685 \pm 0.0028$ & $0.5340 \pm 0.0042$ & $-30.5\%$ & $0.0715 \pm 0.0019$ & $0.2180 \pm 0.0049$ \\
\textbf{CL4KT} \cite{KT039_CL4KT_2022} & Contrastive SSL & $0.7850 \pm 0.0025$ & $0.5412 \pm 0.0039$ & $-31.1\%$ & $0.0620 \pm 0.0016$ & $0.1985 \pm 0.0044$ \\
\textbf{SINKT} \cite{KT044_SINKT_2024} & Inductive GNN + LLM & $\mathbf{0.7920 \pm 0.0022}$ & $\mathbf{0.7185 \pm 0.0041}$ & $\mathbf{-9.3\%}$ & $\mathbf{0.0512 \pm 0.0014}$ & $\mathbf{0.0982 \pm 0.0028}$ \\
\bottomrule
\end{tabular}
}
\vspace{1mm}\\
\raggedright \tbltiny Note: Prototype specification metrics shown for illustration of protocol gap. Formal inferential testing requires expanded independent fold resamplings ($N \ge 10$).
\end{table*}

\begin{table*}[t]
\caption{Taxonomy Table 3: Sparse-Concept \& Cold-Start Evaluation Protocols (RQ4). Representative subset of studies with explicit sparse-concept claims shown; remaining core studies employ generic learner splits without sparse-concept claims.}
...
\midrule
\textbf{KT035} & HHSKT \cite{KT035_HHSKT_2023} & Hierarchical Sparse KC & \textbf{TBD\_VERIFY} & Protocol Under Audit & External Heterogeneous Graph & Reported sparse KC; exact zero-exposure protocol under audit \\
...
\textbf{KT004} & GKT \cite{KT004_GKT_2019} & Interaction Sparsity & \textbf{NO} & Generic Learner Split & Co-occurrence Graph & Evaluates under generic interaction sparsity \\
\textbf{KT010} & GIKT \cite{KT010_GIKT_2021} & Interaction Sparsity & \textbf{NO} & Generic Learner Split & Interaction Graph & Evaluates under generic interaction sparsity \\
\textbf{KT011} & Bi-CLKT \cite{KT011_BiCLKT_2022} & Data Sparsity & \textbf{NO} & Generic Learner Split & Dual Graph & Evaluates under generic interaction sparsity \\
\textbf{KT012} & JKT \cite{KT012_JKT_2021} & Data Sparsity & \textbf{NO} & Generic Learner Split & Similarity Graph & Evaluates under generic interaction sparsity \\
\textbf{KT026} & Pre-train Q-Emb \cite{KT026_PretrainQ_2020} & Data Sparsity & \textbf{NO} & Generic Learner Split & Question Graph & Evaluates under generic interaction sparsity \\
\textbf{KT038} & DyGKT \cite{KT038_DyGKT_2024} & Dynamic Data Sparsity & \textbf{NO} & Generic Learner Split & Dynamic Graph & Evaluates under generic interaction sparsity \\
\textbf{KT039} & CL4KT \cite{KT039_CL4KT_2022} & Interaction Sparsity & \textbf{NO} & Generic Learner Split & Co-occurrence Graph & Evaluates under generic interaction sparsity \\
\textbf{KT066} & HyperKT \cite{KT066_HyperKT_2025} & High-order Data Sparsity & \textbf{NO} & Generic Learner Split & Adaptive Hypergraph & Evaluates under generic interaction sparsity \\
\textbf{KT068} & HKT \cite{KT068_HKT_2025} & Hierarchical Data Sparsity & \textbf{NO} & Generic Learner Split & Hierarchical Graph & Evaluates under generic interaction sparsity \\
\textbf{KT076} & KGNN-KT \cite{KT076_KGNNKT_2025} & Code Data Sparsity & \textbf{NO} & Generic Learner Split & Knowledge Graph + LLM & Evaluates under generic interaction sparsity \\
\textbf{KT077} & R²GCurL \cite{KT077_R2GCurL_2026} & Robust Data Sparsity & \textbf{NO} & Generic Learner Split & Curriculum Graph & Evaluates under generic interaction sparsity \\
\bottomrule
\end{tabular}
}
\end{table*}
...
We systematically audited primary core studies across eight quality criteria (QA1--QA8):
\begin{itemize}
    \item \textbf{QA1 Task Clarity (1.95 / 2.00)}: Consistent problem formulation across studies.
    \item \textbf{QA2 Data Transparency (1.90 / 2.00)}: Broad evaluation on public benchmark datasets (ASSISTments, EdNet, Statics, Junyi).
    \item \textbf{QA3 Split Control (1.78 / 2.00)}: Most studies describe train/test partitions, though graph precomputation leakage risk remains unaddressed in several works.
    \item \textbf{QA4 Baseline Adequacy (1.92 / 2.00)}: Strong baseline comparisons against DKT, SAKT, AKT, and CL4KT.
    \item \textbf{QA5 Statistical Rigor (1.10 / 2.00)}: Significant gap: less than 15\% perform formal significance testing or report confidence intervals.
    \item \textbf{QA6 Reproducibility Artifacts (1.48 / 2.00)}: Publicly accessible code implementations exist for approximately 42\% of studies (either official author repositories or community-verified implementations in benchmarks such as pyKT).
    \item \textbf{QA7 Sparse Construct Validity (1.05 / 2.00)}: Applies specifically to studies claiming sparse-concept or cold-start capabilities; studies with non-sparse claims are marked NA to avoid confounding study scope with quality.
    \item \textbf{QA8 Computational Transparency (1.42 / 2.00)}: Empirical training times, GPU memory consumption, or asymptotic bounds are reported in less than half of published works.
\end{itemize}

\subsection{Asymptotic Computational Complexity \& GPU Memory Footprint}
To assist practitioners in deploying Graph-SSL KT models, we derive asymptotic time complexity bounds across major GNN encoder families:
\begin{enumerate}
    ...
    \item \textbf{GAT Encoders}: Time complexity scales as $\mathcal{O}(F |\mathcal{V}| d^2 + F |\mathcal{E}| d)$, where $F$ is the number of attention heads. Dynamic attention weight storage incurs higher memory overhead.
...
Furthermore, our review observed that \textbf{Expected Calibration Error (ECE) is virtually unreported} across primary core studies in the current literature. To evaluate prediction confidence and demonstrate calibration degradation under cold-start, we computed ECE across $M=10$ equal-width probability bins:
\begin{equation}
...
Based on our synthesis of primary core studies, we formulate six evidence-derived research agenda pillars, providing explicit mathematical formalizations for future research:
...
Establishing standardized benchmark split protocols where cold-start concepts $\mathcal{K}_{\text{cold}} \subset \mathcal{K}$ have zero training exposure:
\begin{equation}
n_{\text{train}}(k) = 0, \quad \forall k \in \mathcal{K}_{\text{cold}}
\end{equation}
with the training dataset defined as:
\begin{equation}
\mathcal{D}_{\text{train}}^{\text{cold}} = \{ (x_t, r_t) \in \mathcal{D}_{\text{train}} : \text{KC}(x_t) \notin \mathcal{K}_{\text{cold}} \}
\end{equation}
and model evaluation conducted strictly on interaction sequences involving concepts in $\mathcal{K}_{\text{cold}}$.
...
\end{equation}
where $T > 0$ is optimized on a validation set via negative log-likelihood (NLL), with Expected Calibration Error (ECE) and Brier scores evaluated post-calibration.
...
Developing causal counterfactual graph frameworks under the backdoor adjustment formula, assuming an admissible adjustment set $Z$ that blocks all backdoor paths from exercise exposure $X$ to student performance $Y$:
\begin{equation}
...
Our meta-synthesis reveals three cross-cutting patterns that transcend individual model contributions. First, the field has undergone a structural transition from static to dynamic graph representations: prior to 2022, over 80\% of Graph-KT models used fixed precomputed adjacency matrices; by 2025--2026, dynamic and jointly learned graphs (DyGKT \cite{KT038_DyGKT_2024}, R$^2$GCurL \cite{KT077_R2GCurL_2026}, STG-SKT \cite{KT079_STGSKT_2026}) represent the dominant paradigm. Second, self-supervised pre-training has progressively decoupled from graph components: early SSL-KT models (CL4KT \cite{KT039_CL4KT_2022}) applied contrastive objectives directly on sequential interaction data, while recent architectures (HyperKT \cite{KT066_HyperKT_2025}, S2-HHN \cite{KT042_S2HHN_2023}) construct dedicated graph-structured contrastive views. Third, LLM integration is emerging as a third paradigm distinct from both Graph-KT and SSL-KT, with SINKT \cite{KT044_SINKT_2024}, Coda \cite{KT070_Coda_2025}, and KGNN-KT \cite{KT076_KGNNKT_2025} using foundation models primarily for cold-start knowledge graph construction rather than sequence prediction.

\subsection{Why Standard KT Models Fail Under Cold-Start}
The prototype benchmark evaluation (Table~\ref{tab:empirical_validation}) illustrates why standard sequence and graph architectures degrade under zero-exposure KC splits. For ID-based sequence models (DKT, CL4KT), unobserved KCs lack trained embedding parameters (an out-of-vocabulary representation failure). For log-derived Graph-KT models (GKT), co-occurrence graph edges between held-out KCs and training KCs are unobserved by construction, so message passing propagates near-zero information to withheld concept nodes. This failure is structurally distinct from generic interaction data sparsity and cannot be resolved by sequence augmentation alone without external structural or semantic priors.

\subsection{Limitations of the Present Survey}
We acknowledge the following limitations. First, study eligibility and coding were conducted via a systematic protocol with cross-checking across pilot waves; formal independent multi-coder inter-rater reliability (Cohen's $\kappa$) represents an ongoing quality enhancement. Second, the empirical proof-of-concept (Table~\ref{tab:empirical_validation}) is an exploratory prototype specification designed for protocol illustration rather than a definitive multi-dataset benchmark. Third, rapidly evolving LLM-KT integration work published after our search window falls outside our synthesis scope.

% ─────────────────────────────────────────────────────────────────────────────
\section{Threats to Validity}

\subsection{Internal Validity}
\textbf{Search completeness}: Despite executing four query families across six core databases and secondary preprint discovery, grey literature was excluded per protocol. Relevant work published in non-indexed venues may be absent from our corpus.

\textbf{Selection bias}: The primary gateway criterion ($\text{PRIMARY\_CORE} = \text{KT} \land (\text{GRAPH} \lor \text{SSL})$) was applied systematically. Borderline cases (e.g., models using attention-based graph structures without explicit GNN components) were adjudicated through protocol consensus review.

\textbf{Coding reliability}: QA scores (QA1--QA8) were assigned following our locked codebook. Formal double-coding and multi-rater agreement metrics over the complete corpus represent ongoing refinements.

\subsection{External Validity}
\textbf{Dataset generalizability}: Most studies are evaluated on standard benchmark datasets (ASSISTments2009/2012/2017, EdNet, Statics2011, Junyi Academy). Findings may not generalize directly to high-stakes assessment platforms or non-English educational environments.

\textbf{Temporal scope}: The rapid emergence of LLM-augmented KT after 2024 indicates that architectural paradigms are evolving rapidly.

\subsection{Construct Validity}
\textbf{Sparse-concept operationalization}: We define strict KC cold-start as zero training-interaction exposure. Studies using mild interaction rarity (e.g., fewer than 5 interactions) as a proxy for sparsity were coded as generic interaction sparsity rather than KC cold-start.
...
This systematic survey provides a comprehensive PRISMA 2020--guided synthesis of Graph-Based and Self-Supervised Knowledge Tracing (KT) under sparse-concept settings. Synthesizing primary core studies published between 2018 and 2026 across six core bibliographic databases and secondary discovery sources, we established three central contributions:

\textbf{Contribution 1 — Two-dimensional taxonomy.} We classified Graph-KT models along a graph provenance axis (Expert Curriculum, Precomputed Log-based, Jointly Learned, LLM-Extracted) and a topological/encoder axis (GCN, GAT, Heterogeneous, Hypergraph, Dynamic, and Explicit Structural Constraints), and classified SSL-KT models into four pretext objective families (Multi-View Contrastive, Graph Structural Contrastive, Hypergraph Contrastive, Generative Masked Pre-training).

\textbf{Contribution 2 — Evaluation split audit.} We clarified the critical distinction between generic interaction data sparsity and strict zero-exposure concept cold-start. While the vast majority of studies motivate their work through data sparsity, only a small fraction isolate true zero-exposure cold-start protocols. Standard sequence and graph architectures degrade substantially under strict cold-start, underscoring that empirical progress has largely been evaluated under lenient regimes that do not reflect unobserved curriculum conditions.
...
We anticipate that this survey will serve as a rigorous reference for researchers in the Graph-SSL KT domain and as a methodological foundation for standardizing evaluation practices across educational data mining.
